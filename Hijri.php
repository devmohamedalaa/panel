...
class Hijri
{
  ...
  static  function ConstractDayMonthYear($date,$format="YYYY/MM/DD") // extract day, month, year out of the date based on the format. 
   ...
   public static  function toMilady($date,$format="YYYY/MM/DD") // $date like 10121400, $format like DDMMYYYY, take date & check if its hijri then convert to gregorian date in format (DD-MM-YYYY), if it gregorian the return empty; 
    { 
...
public static function toHijri($date,$format="YYYY/MM/DD") // $date like 10122011, $format like DDMMYYYY, take date & check if its gregorian then convert to hijri date in format (DD-MM-YYYY), if it hijri the return empty; 
     { 
       self::ConstractDayMonthYear($date,$format); 
        $d=intval(self::$Day); 
        $m=intval(self::$Month); 
